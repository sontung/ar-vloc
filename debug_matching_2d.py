    ...
    if pre_specified:
        ...
        for _idx, _kp in enumerate(tqdm(_kp_list1)):
            ...
            if dis[0] < dis[1]*0.7:
                ...
                _, ind_list2 = _tree.query(_des1[_idx], 5)
                im_show2 = np.copy(im_show)
                color = (random.random()*255, random.random()*255, random.random()*255)
                for idx in ind_list2:
                    x1, y1 = map(int, list(_kp.pt))
                    x2, y2 = map(int, list(_kp_list2[idx].pt))
                    matches.append((x1, y1, x2, y2))
                    cv2.circle(im_show2, (x1, y1), 40, color, -1)
                    cv2.circle(im_show2, (x2 + _img2_show.shape[1], y2), 40, color, -1)
                im_show2 = cv2.resize(im_show2, (im_show2.shape[1] // 4, im_show2.shape[0] // 4))
                cv2.imshow("t", im_show2)
                cv2.waitKey()
